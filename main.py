...
def test_distr_learning(
    ...
    for st_ind in range(train_steps):
        batch = src_distr((batch_size, 1)).repeat(1, src_dist_dim).detach()
...
def test_src_normal_dst_real_nvp_logp(args):
    src_dist = NormalDist(1, 1.2)
    dst_dist = RealNVP(
        dim=2,
    ...
    plot(
        *logger.get_plot_data({
        ...
        histogram_data=[src_samples, dst_samples],
        histogram_titles=['src normal dist', 'dst real nvp dist'],
    )


def test_src_gmm_dst_real_nvp_logp(args):
    # src_dist = GaussianMixtureDist([-4, 2], [1.2, 1])
    src_dist = GaussianMixtureDist([-4, 2, 8], [1.2, 1, 1.5])
    # src_dist = GaussianMixtureDist([-4, 2, 10, 16], [1.2, 1, 0.8, 1.5])
    # src_dist = GaussianMixtureDist([-4, 2, 10, 16, 20, 25], [1.2, 1, 0.8, 1.5, 1, 1.5])
    dst_dist = RealNVP(
        dim=2,
    ...
    plot(
        *logger.get_plot_data({
            'Loss': ['logp_loss'],
            'Mu': [
                'src gaussian mixture mu_0',
                'src gaussian mixture mu_1',
                'src gaussian mixture mu_2',
                'src gaussian mixture mu_3',
            ],
            'Sigma': [
                'src gaussian mixture sigma_0',
                'src gaussian mixture sigma_1',
                'src gaussian mixture sigma_2',
                'src gaussian mixture sigma_3',
            ]
        }),
        histogram_data=[src_samples, dst_samples],
...
def main():
    ...
    # test_src_normal_dst_real_nvp_logp(args)
